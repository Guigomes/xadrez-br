...
export function ImportStandings({ slug }: { slug: string }) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      setResult(data);
      setStatus('success');
      // Invalidate standings queries so the public view updates immediately
  ...
  return (
    <div className="card p-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">Importar classificação</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            Envie o arquivo .xlsx exportado pelo Chess-Results após cada rodada.
          </p>
        </div>
        <button
