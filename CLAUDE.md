...
- **Scripts de aplicação de migration**: `apply-NNN.mjs` (padrão `cron-import/apply-*.mjs`) — conexão `pg` direta. **A `SUPABASE_DB_URL` está no `.env.local`** (gitignored, nunca commitar): host direto `db.<ref>.supabase.co:5432`, que **resolve do sandbox do agente** (tem rota IPv6 — o `ENETUNREACH` só acontece em rede local sem IPv6). Rodar: `node --env-file=.env.local scripts/apply-NNN.mjs` (o script lê `process.env.SUPABASE_DB_URL`, não carrega dotenv sozinho). **`pg` não está nas deps do chess-viewer** — instalar com `npm i pg --no-save` ou usar o de `../cron-import/node_modules`. Alternativas: pooler `postgres.<ref>@aws-1-us-east-2.pooler.supabase.com:6543` (IPv4, mas exige a região certa — deste projeto é `us-east-2`), ou colar o SQL no **SQL Editor** do dashboard. Verificações **read-only** também dá via REST (`curl` com `service_role` key, HTTPS, não depende de IPv6). **Senha do banco vive só no `.env.local`, nunca em arquivo tracked (CLAUDE.md incluso) — vai pro histórico do git e pro GitHub.**
- **Lint**: `npx next lint` deve dar zero *Error* (warnings de `no-explicit-any` são pré-existentes e tolerados). `next.config.js` tem `typescript.ignoreBuildErrors: true` — há ~520 erros de tipo pré-existentes em todo o projeto (cliente Supabase sem generic `Database` efetivo), não é regressão introduzir mais um em código novo, mas não vale tentar zerar isso de uma vez. Causa raiz diagnosticada e caminho de correção em `docs/pendencia-tipos-supabase.md`: o `Database` existe mas não "pega" porque os `Row` são `interface` (não satisfazem `Record<string, unknown>` que o supabase-js exige em `GenericTable`), então toda query resolve pra `never`. Corrigir = regenerar via `supabase gen types` num PR isolado.
...
- **Teste no browser interno do agente (preview pane)**: páginas **server component** (`/`, `/admin`, `/tournaments/...`) renderizam normal, mas páginas **client-gated por auth** (as abas de `/admin/tournaments/[slug]/...`, `/admin/tournaments/new` — que dependem de `useUser`/`useProfile`) ficam presas em "Carregando…" nesse browser sandbox: o `getUser()` client por cookie não resolve (particionamento de cookie/storage). Não é bug do app. Pra validar essas telas, usar Chrome real; no preview pane dá pra validar só o que é server-rendered. Cadastro de teste: `/` → "Cadastre-se" marca "Organizar torneios" → nasce com `is_organizer=true` sem precisar mexer no banco.
...
- **Flash "Entrar" → "Minha conta" no header** (e bolha do Gambito "pipocando"): `Header`/`ChatWidget` derivavam auth só no client (`useUser`, round-trip `getUser` pós-hidratação), montados no layout raiz. Corrigido passando `initialUser` do servidor: `app/layout.tsx` virou `async`, lê `getUser()` e injeta nos dois; enquanto `useUser().loading`, usa o `initialUser` (primeiro paint correto), depois passa pro live (reage a login/logout). Não voltar a montar componente auth-dependente no layout raiz sem esse fallback de servidor.
- **Home `/` sequestrava todo mundo pro último torneio**: `middleware.ts` redirecionava qualquer acesso a `/` pro cookie `last_tournament` (só o logo tinha bypass `?home=1`) — PWA, URL digitada e 404 ficavam presos no torneio. Agora só redireciona **anônimo**; logado tem home própria. `/` server-roteia: organizador (`role=admin` ou `is_organizer`) vê `components/home/organizer-home.tsx` (lista dos próprios torneios), o resto vê `components/home/marketing-home.tsx`. `?home=1` expira o cookie (start_url da PWA usa `?home=1`). Não reintroduzir redirect de `/` pra logado.
- **Ritmo de jogo virou select + coluna `time_control_kind`** (migration 061): `time_control` continua sendo o texto exibido; `time_control_kind` (enum bullet/blitz/rapid/classical/other) é a categoria pra busca/estatística. Presets em `lib/utils/time-control.ts` (fonte única); escolher um preset sugere o `rating_kind` de seed **sem forçar** (só até o organizador mexer manualmente). "Outro" revela o campo de texto livre.
- **Busca de staff por nome/e-mail** (migration 062, `search_staff_candidates`): `user_profiles` tem RLS self-read, então `.ilike()` do client volta vazio — autocomplete precisa de RPC `security definer` gated por `is_tournament_organizer`. Padrão reusável pra qualquer busca em `user_profiles` a partir do client. O RPC de escrita continua `add_staff_by_email` (a busca só preenche o e-mail que ele já consome).
- **Selo cosmético que ignorou o override que o backend já respeitava** (migration 045 → 063): "Reabrir Inscrições" põe `registration_closes_by_date=false` e o status real fica `registration` (aberto), mas `isRegistrationClosed` (`lib/utils/chess.ts`) decidia o rótulo/cor do badge só por `registration_end_date < hoje` — mostrava "Inscrições encerradas" mesmo com o torneio aberto. O estado durava (fix de 045 no banco), o *selo* é que não recebeu o interruptor. Corrigido passando `registration_closes_by_date` até `isRegistrationClosed`/`getTournamentStatusLabel`/`getTournamentStatusColor` (3º param, default `true` = default do banco) em TODOS os call sites (admin/page, organizer-home, players, tournament-card, chrome, layouts público e admin). `search_tournaments` (RPC da lista pública) não projetava o campo — migration 063 só o adiciona à projeção; `TournamentListItem` ganhou o campo. Regra: ao criar uma regra de negócio com "interruptor" (flag que muda o comportamento por data), varrer TODA derivação cosmética daquele estado (labels, cores, ícones) — não só a transição real; senão a UI mente sobre um estado que o backend já corrigiu.
...
- **Arquitetura**: pergunta → embedding (Voyage AI `voyage-4-lite`) → `match_kb_chunks` (pgvector, `kb_chunks`) → contexto no prompt (`lib/chat/prompt.ts`) → LLM gera resposta, com **tool calling** disponível pra 3 ferramentas (`lib/chat/tools.ts`, formato neutro/JSON Schema puro): contar torneios por estado, contar meus torneios, registrar pergunta sem resposta. `lib/chat/llm.ts` implementa o loop pros dois provedores (`generateWithAnthropic`/`generateWithGemini`), cada um adaptando `TOOL_DEFINITIONS` pro formato de tool use específico (Anthropic: `input_schema` quase 1:1; Gemini: `Type` enum via `toGeminiDeclarations()`).
- **Provedor ativo (2026-08-03): Anthropic** (`CHAT_LLM_PROVIDER=anthropic`, `claude-haiku-4-5-20251001`) — decisão do usuário de abandonar o Gemini. Motivo: free tier do Gemini (`gemini-3-flash-preview`) é **só 20 requisições/dia por modelo** (bem mais restrito do que a doc geral do Gemini sugere) — estourava fácil só testando. Código do Gemini continua no repo (arquitetura trocável), sem previsão de uso.
- **Comportamento de tool-calling difere entre modelos**: testado que Claude Haiku só chama `registrar_pergunta_sem_resposta` pra pergunta plausivelmente dentro do domínio mas fora da KB (ex.: "como transferir organização de torneio") — pergunta claramente fora de assunto (ex.: "horário de lotérica") ele responde direto sem chamar a ferramenta, porque já sabe redirecionar sem precisar registrar. Gemini era mais "gatilho fácil", chamando a ferramenta também pro caso óbvio. Ambos os comportamentos são aceitáveis — o de Claude até evita poluir `unanswered_questions` com pergunta claramente fora de escopo.
