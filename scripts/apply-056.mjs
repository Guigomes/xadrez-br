// Aplica supabase/migrations/056_no_auto_status_change_on_creation_day.sql via
// conexão direta Postgres (mesmo padrão de apply-035.mjs / apply-038.mjs /
// apply-046.mjs / apply-047.mjs).
//
// Uso:
//   SUPABASE_DB_URL="postgres://postgres:<senha>@<host>:5432/postgres" node scripts/apply-056.mjs
//
...
const sqlPath = join(__dirname, '..', 'supabase', 'migrations', '056_no_auto_status_change_on_creation_day.sql');
...
async function main() {
  ...
  console.log('Conectado. Aplicando 056_no_auto_status_change_on_creation_day.sql em uma transação…');
  ...
  console.log('\nConferindo assinatura de next_status_by_date (6 parâmetros, com p_created_at)…');
  ...
  const ok = args.includes('p_created_at') && args.split(',').length === 6;
