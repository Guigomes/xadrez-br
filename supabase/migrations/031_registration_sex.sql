-- ============================================================
-- Migration 031: sexo na inscrição pública
-- ============================================================
-- players.sex ('m'/'w') já existe (015) e é usado pela categoria Feminino
-- e pelo export TRF de homologação, mas nada preenchia — a inscrição não
-- coletava sexo. Aqui: coluna sex na inscrição + propagação para players
-- no approve_registration.
...
do $$ begin
  alter table tournament_registrations
    add constraint tournament_registrations_sex_check check (sex in ('m', 'w'));
exception when duplicate_object then null; end $$;

-- Recria approve_registration incluindo sex ao criar/atualizar o player.
...
begin
  ...
  if v_player_id is null then
    ...
    -- completa o sexo se o player já existia sem esse dado
